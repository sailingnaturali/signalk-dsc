...
function normalizeMmsi(value) {
  ...
  // `DSC Message Address` is a 40-bit BCD DECIMAL — ten digits, the 9-digit
  // MMSI with a trailing pad, the same convention as the $CDDSC address field
  // (see parseMmsi in lib/dsc.js). Real gateway traffic always carries the
  // padded form, so rejecting it drops the MMSI on every N2K call.
  const mmsi = digits.length === 10 ? digits.substring(0, 9) : digits;
