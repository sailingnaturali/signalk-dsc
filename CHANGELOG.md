...
- Securité/safety received calls now raise notification state `warn` (was
  `alert`), aligning with the finalized SignalK spec mapping: distress →
  `emergency`, urgency → `alarm`, safety → `warn`.
