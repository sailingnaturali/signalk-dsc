...
test('voice message falls back to MMSI when the vessel name is unknown', () => {
  const msg = buildMessage(DISTRESS_EVENT, { ownPosition: OWN_POSITION });
  assert.equal(
    msg,
    'DSC distress alert: vessel MMSI 338040079, sinking, 2.3 nautical miles northwest. Monitor channel 16.'
  );
});

test('voice message falls back to coordinates without own position', () => {
  const msg = buildMessage(DISTRESS_EVENT, {});
  assert.match(msg, /^DSC distress alert: vessel MMSI 338040079, sinking, position 48°47\.700′N 123°15\.900′W\. Monitor channel 16\.$/);
});
...
test('distress without any position still reads sanely', () => {
  ...
  assert.equal(msg, 'DSC distress alert: vessel MMSI 338040079, man overboard, position unknown. Monitor channel 16.');
});
...
test('urgency call message has no channel-16 action and no nature', () => {
  ...
  assert.equal(msg, 'DSC urgency call: vessel MMSI 366999707, 2.3 nautical miles northwest.');
});
